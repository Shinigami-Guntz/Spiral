        ...
        operator fun invoke(model: SRDIModel, flipUVs: Boolean = false, invertXAxis: Boolean = false, name: String? = null): ColladaPojo {
            val colladaMeshes = model.meshes.mapIndexed { index, mesh ->
                ...
                val verticeSource = ColladaSourcePojo(
                        "vertices_source_mesh_${index}", "vertices_array_${mesh.name}",
                ...
                val textureSource = ColladaSourcePojo(
                        "uv_source_mesh_${index}", "uv_array_${mesh.name}",
                ...
                val normalSource = ColladaSourcePojo(
                        "normals_source_mesh_${index}", "normals_array_${mesh.name}",
                ...
                return@mapIndexed ColladaGeometryPojo(id = "mesh_$index", name = mesh.name, mesh = ColladaMeshPojo(listOf(verticeSource, textureSource, normalSource), verticesPojo, listOf(triangles)))
            }

            val collada = ColladaPojo(
                    asset = ColladaAssetPojo(contributor = ColladaContributorPojo(authoring_tool = "SPIRAL v${SpiralData.version
                            ?: "Developer"}", comments = "Autogenerated from $name"), up_axis = ColladaUpAxis.Y_UP),
